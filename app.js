...
const SEED_VERSION=window.TRIP_SEED_VERSION||'0';
function seedDays(){return JSON.parse(JSON.stringify(window.DEFAULT_TRIP_DAYS||[]))}
let DAYS=seedDays();
...
// seed에 rev 마커가 붙은 날은 저장/원격 데이터에도 1회 반영 (rev가 다를 때만 교체)
function migrateRevDays(days){(window.DEFAULT_TRIP_DAYS||[]).forEach(sd=>{if(!sd.rev)return;const i=days.findIndex(d=>d&&d.date===sd.date);if(i>-1&&days[i].rev!==sd.rev)days[i]=JSON.parse(JSON.stringify(sd))});return days}
function migrateStatus(days){migrateRevDays(days);days.forEach(day=>day.schedule.forEach(item=>{
...
async function fetchRemoteThenSync(){const ref=getDataRef();if(!ref)return;updateSyncUI('syncing');try{const snap=await ref.once('value');const r=snap.val();if(r&&r.days&&r.updatedAt&&r.seedVersion===SEED_VERSION){DAYS=migrateStatus(r.days);saveToLocalOnly();lastSyncTimestamp=r.updatedAt;if(currentDay>=DAYS.length)currentDay=DAYS.length-1;render();updateSyncUI('online');showToast('클라우드 데이터 로드')}else{
// 클라우드가 비었거나 seed 버전이 다름(구버전) → 최신 seed 로 교체 후 클라우드 덮어쓰기
const stale=!!(r&&r.days);DAYS=migrateStatus(seedDays());saveToLocalOnly();if(currentDay>=DAYS.length)currentDay=DAYS.length-1;render();syncToRemote();if(stale)showToast('☁️ 클라우드 일정을 최신본으로 갱신했어요')}}catch(e){updateSyncUI('offline')}}
function syncToRemote(){if(!fbDb||!fbUser||isSyncingFromRemote)return;const ref=getDataRef();if(!ref)return;updateSyncUI('syncing');ref.set({days:DAYS,seedVersion:SEED_VERSION,updatedBy:fbUser.uid,updatedByName:fbUser.displayName||fbUser.email,updatedAt:firebase.database.ServerValue.TIMESTAMP}).then(()=>{lastSyncTimestamp=Date.now();updateSyncUI('online')}).catch(()=>updateSyncUI('offline'))}
function listenForChanges(){detachListeners();const ref=getDataRef();if(!ref)return;dataListener=ref.on('value',snap=>{const data=snap.val();if(!data||!data.days)return;if(data.updatedBy===fbUser?.uid)return;if(data.seedVersion!==SEED_VERSION){console.warn('구버전 seed 데이터 수신 — 무시하고 최신본 재업로드',data.seedVersion);syncToRemote();return}if(editMode){pendingRemoteData=data;showToast(`📡 ${data.updatedByName||'상대방'}님 수정 — 편집 후 반영`);return}const oldDays=JSON.stringify(DAYS);isSyncingFromRemote=true;DAYS=migrateStatus(data.days);saveToLocalOnly();lastSyncTimestamp=data.updatedAt||Date.now();if(currentDay>=DAYS.length)currentDay=DAYS.length-1;render();highlightConflicts(oldDays,data.days);showToast(`📡 ${data.updatedByName||'상대방'}님이 수정`);isSyncingFromRemote=false})}
...
// ══════════ PERSISTENCE ══════════
function saveToLocalOnly(){try{localStorage.setItem('travel_planner_days',JSON.stringify(DAYS));localStorage.setItem('travel_seed_version',SEED_VERSION)}catch(e){if(e&&e.name==='QuotaExceededError')showToast('저장소 용량 초과 — 사진/메모를 줄여주세요');else console.warn('saveToLocal 실패:',e)}}
function saveToLocal(){saveToLocalOnly();syncToRemote()}
function loadFromLocal(){try{if(localStorage.getItem('travel_seed_version')!==SEED_VERSION){console.info('seed 버전 변경 — 로컬 저장본 대신 최신 일정 사용');return}const d=localStorage.getItem('travel_planner_days');if(d){const parsed=JSON.parse(d);if(Array.isArray(parsed)&&parsed.length)DAYS=parsed}}catch(e){console.warn('loadFromLocal 실패 — seed 데이터 사용:',e);showToast('저장 데이터 손상 — 기본값 복구')}}
...
if(mapMode==='day'){const day=DAYS[currentDay];const coords=[];let num=1;day.schedule.forEach(item=>{if(!item.coords)return;const[lat,lng]=item.coords;coords.push([lat,lng]);const t=TYPE_STYLES[item.type]||TYPE_STYLES.etc;const icon=L.divIcon({className:'',html:`<div class="custom-marker" style="background:${t.dot}">${num}</div>`,iconSize:[26,26],iconAnchor:[13,13]});const pvid=voucherIdFor(item);const ppn=extractPlaceName(item,day);
const mk=L.marker([lat,lng],{icon}).bindPopup(`<div class="mpop">
<div class="mpop-head">${item.time!=='—'?`<span class="mpop-time">${item.time}</span>`:''}<span class="mpop-type" style="background:${t.dot}1f;color:${t.dot}">${t.label}</span>${item.status&&STATUS_LABELS[item.status]?`<span class="status-badge ${STATUS_CSS[item.status]}">${STATUS_LABELS[item.status]}</span>`:''}</div>
<div class="mpop-title">${esc(item.title)}</div>
${item.desc?`<div class="mpop-desc">${esc(item.desc)}</div>`:''}
${item.warn?`<div class="mpop-warn">⚠️ ${esc(item.warn)}</div>`:''}
<div class="mpop-actions">${pvid?`<button class="mpop-btn mpop-voucher" onclick="openVoucherFromCard('${pvid}')">🎫 바우처</button>`:''}<a class="mpop-btn mpop-review" href="${gmapsUrl(item.coords,ppn)}" target="_blank">⭐ 리뷰</a><a class="mpop-btn mpop-dir" href="${gmapsDirUrl(item.coords,ppn)}" target="_blank">🧭 길찾기</a></div>
</div>`,{maxWidth:300,minWidth:230,className:'mpop-wrap'}).addTo(map);mapMarkers.push(mk);num++});if(coords.length>=2){// Show dashed line immediately, then overlay real route
...
// ══════════ PHRASE CHEATSHEET ══════════
let phraseLang='es',phraseSection='all',phraseQuery='';
const PHRASES={es:{name:'스페인어 🇪🇸',sections:{'👋 인사/기본':[
{ko:'안녕하세요',local:'Hola',pron:'올라'},{ko:'감사합니다',local:'Gracias',pron:'그라시아스'},{ko:'천만에요',local:'De nada',pron:'데 나다'},{ko:'부탁합니다/제발',local:'Por favor',pron:'뽀르 파보르'},{ko:'네/아니요',local:'Sí / No',pron:'시 / 노'},{ko:'실례합니다',local:'Perdón / Disculpe',pron:'뻬르돈 / 디스꿀뻬'},{ko:'죄송합니다',local:'Lo siento',pron:'로 시엔또'},{ko:'좋은 하루!',local:'¡Buen día!',pron:'부엔 디아'},{ko:'안녕히 가세요',local:'Adiós',pron:'아디오스'},{ko:'영어 가능하세요?',local:'¿Habla inglés?',pron:'아블라 잉글레스?'},{ko:'이해 못했어요',local:'No entiendo',pron:'노 엔띠엔도'},{ko:'천천히 말해주세요',local:'Más despacio, por favor',pron:'마스 데스빠시오, 뽀르 파보르'},{ko:'한국에서 왔어요',local:'Somos de Corea',pron:'소모스 데 꼬레아'},{ko:'저희 신혼여행 중이에요',local:'Estamos de luna de miel',pron:'에스따모스 데 루나 데 미엘'}],
'🍽️ 식당':[
{ko:'메뉴 주세요',local:'La carta, por favor',pron:'라 까르따, 뽀르 파보르'},{ko:'이거 주세요',local:'Esto, por favor',pron:'에스또, 뽀르 파보르'},{ko:'추천 메뉴가 뭐예요?',local:'¿Qué me recomienda?',pron:'께 메 레꼬미엔다?'},{ko:'계산서 주세요',local:'La cuenta, por favor',pron:'라 꾸엔따, 뽀르 파보르'},{ko:'맛있어요!',local:'¡Está delicioso!',pron:'에스따 델리시오소!'},{ko:'물 주세요',local:'Agua, por favor',pron:'아구아, 뽀르 파보르'},{ko:'수돗물 주세요 (무료)',local:'Agua del grifo, por favor',pron:'아구아 델 그리포, 뽀르 파보르'},{ko:'맥주/와인 한잔',local:'Una cerveza / Un vino',pron:'우나 세르베사 / 운 비노'},{ko:'샹그리아/까바 한 잔',local:'Una sangría / Una copa de cava',pron:'우나 상그리아 / 우나 꼬빠 데 까바'},{ko:'커피 두 잔 주세요',local:'Dos cafés, por favor',pron:'도스 까페스, 뽀르 파보르'},{ko:'예약했습니다',local:'Tengo una reserva',pron:'뗑고 우나 레세르바'},{ko:'2명이요',local:'Para dos, por favor',pron:'빠라 도스, 뽀르 파보르'},{ko:'테라스 자리 있나요?',local:'¿Hay mesa en la terraza?',pron:'아이 메사 엔 라 떼라사?'},{ko:'포장 되나요?',local:'¿Para llevar?',pron:'빠라 예바르?'},{ko:'화장실 어디예요?',local:'¿Dónde está el baño?',pron:'돈데 에스따 엘 바뇨?'}],
'🏨 숙소':[
{ko:'체크인 할게요 (예약했어요)',local:'Tengo una reserva',pron:'뗑고 우나 레세르바'},{ko:'짐 맡길 수 있나요?',local:'¿Puedo dejar el equipaje?',pron:'뿌에도 데하르 엘 에끼빠헤?'},{ko:'와이파이 비밀번호가 뭐예요?',local:'¿Cuál es la contraseña del wifi?',pron:'꾸알 에스 라 꼰뜨라세냐 델 위피?'},{ko:'조식은 몇 시예요?',local:'¿A qué hora es el desayuno?',pron:'아 께 오라 에스 엘 데사유노?'},{ko:'늦은 체크아웃 되나요?',local:'¿Es posible salida tardía?',pron:'에스 뽀시블레 살리다 따르디아?'},{ko:'수건 더 주세요',local:'Más toallas, por favor',pron:'마스 또아야스, 뽀르 파보르'},{ko:'에어컨이 안 돼요',local:'El aire acondicionado no funciona',pron:'엘 아이레 아꼰디시오나도 노 푼시오나'},{ko:'택시 불러주시겠어요?',local:'¿Puede pedirme un taxi?',pron:'뿌에데 뻬디르메 운 딱시?'}],
'🚌 교통/이동':[
{ko:'~어디에 있나요?',local:'¿Dónde está...?',pron:'돈데 에스따...?'},{ko:'얼마나 걸려요?',local:'¿Cuánto tiempo?',pron:'꾸안또 띠엠뽀?'},{ko:'공항으로 가주세요',local:'Al aeropuerto, por favor',pron:'알 아에로뿌에르또, 뽀르 파보르'},{ko:'이 버스 ~에 가나요?',local:'¿Este autobús va a...?',pron:'에스떼 아우또부스 바 아...?'},{ko:'표 두 장 주세요',local:'Dos billetes, por favor',pron:'도스 비예떼스, 뽀르 파보르'},{ko:'지하철역이 어디예요?',local:'¿Dónde está el metro?',pron:'돈데 에스따 엘 메뜨로?'},{ko:'택시 불러주세요',local:'¿Puede llamar un taxi?',pron:'뿌에데 야마르 운 딱시?'},{ko:'여기서 세워주세요',local:'Pare aquí, por favor',pron:'빠레 아끼, 뽀르 파보르'},{ko:'~까지 얼마예요?',local:'¿Cuánto cuesta hasta...?',pron:'꾸안또 꾸에스따 아스따...?'},{ko:'왼쪽/오른쪽',local:'Izquierda / Derecha',pron:'이스끼에르다 / 데레차'}],
'🚗 렌터카/주유':[
{ko:'주유소가 어디예요?',local:'¿Dónde hay una gasolinera?',pron:'돈데 아이 우나 가솔리네라?'},{ko:'가득 채워주세요',local:'Lleno, por favor',pron:'예노, 뽀르 파보르'},{ko:'무연 휘발유 95 (렌트카용)',local:'Gasolina 95 sin plomo',pron:'가솔리나 노벤따 이 신꼬 신 쁠로모'},{ko:'카드로 계산할게요',local:'Con tarjeta, por favor',pron:'꼰 따르헤따, 뽀르 파보르'},{ko:'주차장이 어디예요?',local:'¿Dónde está el aparcamiento?',pron:'돈데 에스따 엘 아빠르까미엔또?'},{ko:'여기 주차해도 되나요?',local:'¿Puedo aparcar aquí?',pron:'뿌에도 아빠르까르 아끼?'},{ko:'차를 반납하러 왔어요',local:'Vengo a devolver el coche',pron:'벵고 아 데볼베르 엘 꼬체'}],
'📸 사진':[
{ko:'사진 좀 찍어주시겠어요?',local:'¿Nos puede sacar una foto?',pron:'노스 뿌에데 사까르 우나 포또?'},{ko:'한 장 더 부탁해요',local:'Una más, por favor',pron:'우나 마스, 뽀르 파보르'},{ko:'세로로/가로로 찍어주세요',local:'En vertical / En horizontal',pron:'엔 베르띠깔 / 엔 오리손딸'},{ko:'여기(버튼) 눌러주세요',local:'Pulse aquí',pron:'뿔세 아끼'},{ko:'배경까지 나오게 해주세요',local:'Con el fondo, por favor',pron:'꼰 엘 폰도, 뽀르 파보르'},{ko:'찍어드릴까요?',local:'¿Les saco una foto?',pron:'레스 사꼬 우나 포또?'}],
'🛍️ 쇼핑':[
{ko:'얼마예요?',local:'¿Cuánto cuesta?',pron:'꾸안또 꾸에스따?'},{ko:'할인 되나요?',local:'¿Tiene descuento?',pron:'띠에네 데스꾸엔또?'},{ko:'카드 되나요?',local:'¿Aceptan tarjeta?',pron:'아셉딴 따르헤따?'},{ko:'택스리펀 되나요?',local:'¿Tax free?',pron:'탁스 프리?'},{ko:'이거 볼 수 있을까요?',local:'¿Puedo ver esto?',pron:'뿌에도 베르 에스또?'},{ko:'그냥 구경 중이에요',local:'Solo estoy mirando',pron:'솔로 에스또이 미란도'},{ko:'다른 색 있나요?',local:'¿Tiene otro color?',pron:'띠에네 오뜨로 꼴로르?'},{ko:'더 큰/작은 사이즈 있나요?',local:'¿Talla más grande / pequeña?',pron:'따야 마스 그란데 / 뻬께냐?'},{ko:'이거 두 개 주세요',local:'Dos de estos, por favor',pron:'도스 데 에스또스, 뽀르 파보르'},{ko:'봉투 주세요',local:'Una bolsa, por favor',pron:'우나 볼사, 뽀르 파보르'}],
'🔢 숫자/단어':[
{ko:'1 · 2 · 3 · 4 · 5',local:'uno · dos · tres · cuatro · cinco',pron:'우노 · 도스 · 뜨레스 · 꾸아뜨로 · 신꼬'},{ko:'6 · 7 · 8 · 9 · 10',local:'seis · siete · ocho · nueve · diez',pron:'세이스 · 시에떼 · 오초 · 누에베 · 디에스'},{ko:'오늘 / 내일',local:'hoy / mañana',pron:'오이 / 마냐나'},{ko:'열림 / 닫힘',local:'abierto / cerrado',pron:'아비에르또 / 세라도'},{ko:'입구 / 출구',local:'entrada / salida',pron:'엔뜨라다 / 살리다'},{ko:'뜨거운 / 차가운',local:'caliente / frío',pron:'깔리엔떼 / 프리오'}],
'🚨 긴급':[
{ko:'도와주세요!',local:'¡Ayuda!',pron:'아유다!'},{ko:'소매치기예요!',local:'¡Al ladrón!',pron:'알 라드론!'},{ko:'경찰을 불러주세요',local:'Llame a la policía',pron:'야메 아 라 뽈리시아'},{ko:'병원이 어디에요?',local:'¿Dónde está el hospital?',pron:'돈데 에스따 엘 오스삐딸?'},{ko:'약국이 어디예요?',local:'¿Dónde hay una farmacia?',pron:'돈데 아이 우나 파르마시아?'},{ko:'아파요',local:'Me duele',pron:'메 두엘레'},{ko:'여권을 잃어버렸어요',local:'He perdido mi pasaporte',pron:'에 뻬르디도 미 빠사뽀르떼'},{ko:'카드를 분실했어요',local:'He perdido mi tarjeta',pron:'에 뻬르디도 미 따르헤따'},{ko:'유럽 긴급전화 112',local:'Emergencias: 112',pron:''},{ko:'대사관 전화번호',local:'Embajada de Corea: +34 91 353 2000',pron:''}]}},
pt:{name:'포르투갈어 🇵🇹',sections:{'👋 인사/기본':[
{ko:'안녕하세요',local:'Olá',pron:'올라'},{ko:'감사합니다',local:'Obrigado(a)',pron:'오브리가두(다)'},{ko:'천만에요',local:'De nada',pron:'드 나다'},{ko:'부탁합니다',local:'Por favor',pron:'뽀르 파보르'},{ko:'네/아니요',local:'Sim / Não',pron:'심 / 나웅'},{ko:'실례합니다',local:'Desculpe',pron:'데스쿨프'},{ko:'안녕히 가세요',local:'Adeus',pron:'아데우스'},{ko:'영어 가능하세요?',local:'Fala inglês?',pron:'팔라 잉글레스?'},{ko:'이해 못했어요',local:'Não entendo',pron:'나웅 엔뗀두'},{ko:'천천히 말해주세요',local:'Mais devagar, por favor',pron:'마이스 드바가르, 뽀르 파보르'},{ko:'한국에서 왔어요',local:'Somos da Coreia',pron:'소무스 다 꼬레이아'},{ko:'저희 신혼여행 중이에요',local:'Estamos em lua de mel',pron:'이스따무스 엥 루아 드 멜'}],
'🍽️ 식당':[
{ko:'메뉴 주세요',local:'A ementa, por favor',pron:'아 에멘따, 뽀르 파보르'},{ko:'이거 주세요',local:'Isto, por favor',pron:'이스뚜, 뽀르 파보르'},{ko:'추천 메뉴가 뭐예요?',local:'O que recomenda?',pron:'우 끄 헤꼬멘다?'},{ko:'계산서 주세요',local:'A conta, por favor',pron:'아 꼰따, 뽀르 파보르'},{ko:'맛있어요!',local:'Está delicioso!',pron:'이스따 델리시오주!'},{ko:'물 주세요',local:'Água, por favor',pron:'아구아, 뽀르 파보르'},{ko:'맥주/와인 한잔',local:'Uma cerveja / Um vinho',pron:'우마 세르베자 / 움 비뉴'},{ko:'포트와인 한 잔',local:'Um copo de vinho do Porto',pron:'움 꼬뿌 드 비뉴 두 뽀르뚜'},{ko:'에그타르트 두 개 주세요',local:'Dois pastéis de nata, por favor',pron:'도이스 빠스떼이스 드 나따, 뽀르 파보르'},{ko:'커피 두 잔 주세요',local:'Dois cafés, por favor',pron:'도이스 까페스, 뽀르 파보르'},{ko:'예약했습니다',local:'Tenho uma reserva',pron:'떼뉴 우마 헤제르바'},{ko:'2명이요',local:'Para dois, por favor',pron:'빠라 도이스, 뽀르 파보르'},{ko:'포장 되나요?',local:'É para levar',pron:'에 빠라 르바르'},{ko:'화장실 어디예요?',local:'Onde é a casa de banho?',pron:'온드 에 아 까자 드 바뉴?'}],
'🏨 숙소':[
{ko:'체크인 할게요 (예약했어요)',local:'Tenho uma reserva',pron:'떼뉴 우마 헤제르바'},{ko:'짐 맡길 수 있나요?',local:'Posso deixar a bagagem?',pron:'뽀수 데이샤르 아 바가젱?'},{ko:'와이파이 비밀번호가 뭐예요?',local:'Qual é a senha do wi-fi?',pron:'꽐 에 아 세냐 두 위피?'},{ko:'조식은 몇 시예요?',local:'A que horas é o pequeno-almoço?',pron:'아 끄 오라스 에 우 뻬께누 알모수?'},{ko:'수건 더 주세요',local:'Mais toalhas, por favor',pron:'마이스 또알랴스, 뽀르 파보르'},{ko:'택시 불러주시겠어요?',local:'Pode chamar um táxi?',pron:'뽀드 샤마르 움 딱시?'}],
'🚌 교통/이동':[
{ko:'~어디에 있나요?',local:'Onde é...?',pron:'온드 에...?'},{ko:'얼마나 걸려요?',local:'Quanto tempo?',pron:'꽌뚜 뗌뿌?'},{ko:'공항으로 가주세요',local:'Para o aeroporto, por favor',pron:'빠라 우 아에로뽀르뚜, 뽀르 파보르'},{ko:'이 버스/트램 ~에 가나요?',local:'Este autocarro vai para...?',pron:'에스뜨 아우또까후 바이 빠라...?'},{ko:'표 두 장 주세요',local:'Dois bilhetes, por favor',pron:'도이스 빌례뜨스, 뽀르 파보르'},{ko:'여기서 세워주세요',local:'Pare aqui, por favor',pron:'빠르 아끼, 뽀르 파보르'},{ko:'~까지 얼마예요?',local:'Quanto custa até...?',pron:'꽌뚜 꾸스따 아떼...?'},{ko:'왼쪽/오른쪽',local:'Esquerda / Direita',pron:'이스께르다 / 디레이따'}],
'📸 사진':[
{ko:'사진 좀 찍어주시겠어요?',local:'Pode tirar-nos uma foto?',pron:'뽀드 띠라르-노스 우마 포뚜?'},{ko:'한 장 더 부탁해요',local:'Mais uma, por favor',pron:'마이스 우마, 뽀르 파보르'},{ko:'세로로/가로로 찍어주세요',local:'Na vertical / Na horizontal',pron:'나 베르띠깔 / 나 오리존딸'},{ko:'여기(버튼) 눌러주세요',local:'Carregue aqui',pron:'까헤그 아끼'},{ko:'배경까지 나오게 해주세요',local:'Com o fundo, por favor',pron:'꽁 우 푼두, 뽀르 파보르'},{ko:'찍어드릴까요?',local:'Quer que tire uma foto?',pron:'께르 끄 띠르 우마 포뚜?'}],
'🛍️ 쇼핑':[
{ko:'얼마예요?',local:'Quanto custa?',pron:'꽌뚜 꾸스따?'},{ko:'할인 되나요?',local:'Tem desconto?',pron:'뗑 데스꼰뚜?'},{ko:'카드 되나요?',local:'Aceita cartão?',pron:'아세이따 까르따웅?'},{ko:'택스리펀 되나요?',local:'Tem tax free?',pron:'뗑 탁스 프리?'},{ko:'이거 볼 수 있을까요?',local:'Posso ver isto?',pron:'뽀수 베르 이스뚜?'},{ko:'그냥 구경 중이에요',local:'Estou só a ver',pron:'이스또우 소 아 베르'},{ko:'봉투 주세요',local:'Um saco, por favor',pron:'움 사꾸, 뽀르 파보르'}],
'🔢 숫자/단어':[
{ko:'1 · 2 · 3 · 4 · 5',local:'um · dois · três · quatro · cinco',pron:'움 · 도이스 · 뜨레스 · 꽈뜨루 · 신꾸'},{ko:'6 · 7 · 8 · 9 · 10',local:'seis · sete · oito · nove · dez',pron:'세이스 · 세뜨 · 오이뚜 · 노브 · 데스'},{ko:'오늘 / 내일',local:'hoje / amanhã',pron:'오즈 / 아마냥'},{ko:'열림 / 닫힘',local:'aberto / fechado',pron:'아베르뚜 / 페샤두'},{ko:'입구 / 출구',local:'entrada / saída',pron:'엔뜨라다 / 사이다'},{ko:'뜨거운 / 차가운',local:'quente / frio',pron:'껜뜨 / 프리우'}],
'🚨 긴급':[
{ko:'도와주세요!',local:'Ajuda!',pron:'아주다!'},{ko:'소매치기예요!',local:'Ladrão!',pron:'라드라웅!'},{ko:'경찰을 불러주세요',local:'Chame a polícia',pron:'샤미 아 뽈리시아'},{ko:'병원이 어디에요?',local:'Onde é o hospital?',pron:'온드 에 우 오스피따우?'},{ko:'약국이 어디예요?',local:'Onde há uma farmácia?',pron:'온드 아 우마 파르마시아?'},{ko:'아파요',local:'Estou doente',pron:'이스또우 도엔뜨'},{ko:'여권을 잃어버렸어요',local:'Perdi o meu passaporte',pron:'뻬르디 우 메우 빠사뽀르뜨'},{ko:'카드를 분실했어요',local:'Perdi o meu cartão',pron:'뻬르디 우 메우 까르따웅'},{ko:'유럽 긴급전화 112',local:'Emergência: 112',pron:''},{ko:'대사관 전화번호',local:'Embaixada da Coreia: +351 21 793 7200',pron:''}]}}};
function phraseCardHtml(p){const say=p.local.replace(/'/g,"\\'").replace(/"/g,'&quot;');
return`<div class="phrase-card" onclick="if(window.speechSynthesis){const u=new SpeechSynthesisUtterance('${say}');u.lang='${phraseLang}';speechSynthesis.speak(u)}"><div class="phrase-ko">${esc(p.ko)}</div><div class="phrase-local">${esc(p.local)}</div>${p.pron?`<div class="phrase-pron">🔊 ${esc(p.pron)}</div>`:''}</div>`}
function renderPhraseList(){const box=document.getElementById('phraseList');if(!box)return;const lang=PHRASES[phraseLang];
const q=(phraseQuery||'').trim().toLowerCase();
let entries=Object.entries(lang.sections);
if(phraseSection!=='all')entries=entries.filter(([t])=>t===phraseSection);
let total=0;
const html=entries.map(([title,phrases])=>{
const list=q?phrases.filter(p=>(p.ko+' '+p.local+' '+(p.pron||'')).toLowerCase().includes(q)):phrases;
if(!list.length)return'';total+=list.length;
return`<div class="phrase-section"><div class="phrase-section-title">${title} <span class="phrase-sec-cnt">${list.length}</span></div>${list.map(phraseCardHtml).join('')}</div>`}).join('');
box.innerHTML=html||`<div class="phrase-empty">🔍 "${esc(phraseQuery)}" 검색 결과가 없습니다</div>`;
const cnt=document.getElementById('phraseCount');if(cnt)cnt.textContent=q?`${total}개`:''}
function setPhraseSection(s){phraseSection=s;renderPhraseView()}
function clearPhraseSearch(){phraseQuery='';const i=document.getElementById('phraseSearch');if(i){i.value='';i.focus()}renderPhraseList()}
function renderPhraseView(){const el=document.getElementById('phraseView');const lang=PHRASES[phraseLang];
if(phraseSection!=='all'&&!lang.sections[phraseSection])phraseSection='all';
...
const secKeys=Object.keys(lang.sections);
const chips=`<div class="phrase-sec-chips"><button class="phrase-sec-chip ${phraseSection==='all'?'active':''}" onclick="setPhraseSection('all')">전체</button>${secKeys.map(k=>`<button class="phrase-sec-chip ${phraseSection===k?'active':''}" onclick="setPhraseSection('${k}')">${k}</button>`).join('')}</div>`;
const search=`<div class="phrase-search-wrap"><span class="phrase-search-icon">🔍</span><input class="phrase-search-input" id="phraseSearch" placeholder="회화 검색 (한국어 · 현지어 · 발음)" value="${esc(phraseQuery)}" oninput="phraseQuery=this.value;renderPhraseList()"><span class="phrase-count" id="phraseCount"></span><button class="phrase-search-clear" onclick="clearPhraseSearch()">✕</button></div>`;
el.innerHTML=`<div class="prep-title">🗣️ 현지어 치트시트</div>${tabs}${chips}${search}<div style="font-size:10px;color:#64748b;margin-bottom:12px">💡 카드를 탭하면 발음을 들을 수 있어요 (TTS)</div><div id="phraseList"></div>`;
renderPhraseList()}
